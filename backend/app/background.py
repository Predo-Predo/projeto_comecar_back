...
def publicar_app_na_playstore(app_id: int):
    # Envia tarefa para rodar no event loop
    asyncio.run(_publicar_app_async(app_id))


async def _publicar_app_async(app_id: int):
    async with AsyncSessionLocal() as db:
        try:
            result_app = await db.execute(select(models.App).where(models.App.id == app_id))
            ...
            result_empresa = await db.execute(select(models.Empresa).where(models.Empresa.id == app.empresa_id))
            empresa = result_empresa.scalars().first()
            result_projeto = await db.execute(select(models.Projeto).where(models.Projeto.id == app.projeto_id))
